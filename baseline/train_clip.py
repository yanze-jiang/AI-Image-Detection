...
def main() -> None:
    ...
    output_dir = args.output_dir or Path("baseline/outputs/clip") / run_name
    ...
    summary = build_record_base(
        ...
        seed=args.seed,
        device=device,
        script_path="baseline/train_clip.py",
    )
    summary["experiment_id"] = f"clip_{run_name}_train_and_eval"
    summary["run_id"] = f"clip_{run_name}"
    summary["data"].update(
        {
    ...
    summary["clip_model_name"] = args.model_name
